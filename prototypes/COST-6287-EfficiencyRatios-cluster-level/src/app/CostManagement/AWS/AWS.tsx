...
const AWS: React.FunctionComponent = () => {
  ...
  // Get buffer configuration for AWS from localStorage
      ...
      if (savedConfig) {
        ...
        if (config.bufferMode === 'default') {
          ...
          if (config.customMode === 'all') {
            ...
            return {
              before: parseInt(config.providerBuffers?.aws?.before || '3', 10),
              after: parseInt(config.providerBuffers?.aws?.after || '3', 10),
            };
          }
        }
      }
  ...
  const accounts: AccountItem[] = dbAccounts.map(account => {
    ...
    const percentage = (displayCost / totalAWSCost) * 100;
